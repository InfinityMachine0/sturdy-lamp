...
def create_link [ type: string, module: string, git_repo_file: string config_file: string ] {
...
def create_system_link [ module: string, git_repo_file: string config_file: string ] {
	create_link "system" $module $git_repo_file $config_file
}

def create_home_manager_link [ module: string, git_repo_file: string config_file: string ] {
	create_link "home-manager" $module $git_repo_file $config_file
}

def create_basic_system_link [ module: string ] {
	let file_name = ( [ $module, "_config.nix" ] | str join )
	create_system_link $module $file_name $file_name
}

def create_basic_home_manager_link [ module: string ] {
	let file_name = ( [ $module, "_config.nix" ] | str join )
...
def copy_file [ type: string, module: string, git_repo_file: string, config_file: string ] {
	let path_to_git_repo_module_file = ( [ $path_to_git_repo, $type, "modules", $module, $git_repo_file ] | path join )
	let path_to_config_module_file = ( [ $path_to_config, $type, "modules", $module, $config_file ] | path join )
	doas cp $path_to_git_repo_module_file $path_to_config_module_file
}

def copy_system_file [ module: string, git_repo_file: string config_file: string ] {
	copy_file "system" $module $git_repo_file $config_file
}

def copy_home_manager_file [ module: string, git_repo_file: string config_file: string ] {
	copy_file "home-manager" $module $git_repo_file $config_file
}

#######################################

def replace_string [ type: string, module: string, config_file: string, string_to_replace: string, string_replacment: string ] {
	let path_to_config_module_file = ( [ $path_to_config, $type, "modules", $module, $config_file ] | path join )

	doas open --raw $path_to_config_module_file | str replace $string_to_replace $string_replacment | save $path_to_config_module_file
}

def replace_system_string [ module: string, config_file: string, string_to_replace: string, string_replacment: string ] {
	replace_string "system" $module $config_file $string_to_replace $string_replacment
}

def replace_home_manager_string [ module: string, config_file: string, string_to_replace: string, string_replacment: string ] {
	replace_string "home-manager" $module $config_file $string_to_replace $string_replacment
}

#######################################

def select_platform [ platform: int ] {
	if $platform == 1 {
